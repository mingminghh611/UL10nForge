#!/usr/bin/env python3
"""UL10nForge 0.51.0 三通道发行打包（2026-09-09）。
...
模型文件 0.37.1→0.51.0 无变化，代码更新不影响模型包）。

    python scripts/_package_0510.py              # 打包 + 验证 + SHA256
    python scripts/_package_0510.py --verify     # 只验证已有分卷完整性
    python scripts/_package_0510.py --reuse-models   # Models 复用 0.37.1 分卷（重命名）
"""
...
def _reuse_models() -> bool:
    """复用 0.37.1 Models 分卷（模型文件 0.37.1→0.51.0 无变化）：
    复制旧卷为 0.51.0 名并核对分卷数/校验。返回是否成功。
    """
