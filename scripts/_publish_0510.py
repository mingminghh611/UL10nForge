...
距离首个公开测试版 **0.30.0**（2026-08-15）一个多月，期间迭代 20+ 个内部版本、修复上百个真实游戏实测问题、回归测试增长到 **3433 项**。这一版的主线可以概括成三句话：**写回更安全、识别更全、跑得更快更稳**——外加一个全新的在线 API 模式。
...
### 2️⃣ 识别能力：覆盖率 98.5%
- **AI 辅助召回**：确定性规则拿不准的文本交给模型结合现场证据判定，真实游戏语料实测覆盖率 98.5%（0.30.0 时代大量「看起来像代码」的按钮文本 / 短对话被误跳过）
- **引擎字符串吸收层**：il2cpp 引擎日志 / 异常串等 1095 条污染源根除，不再混进翻译池浪费算力
...
### 4️⃣ 性能与内存：大游戏不再劝退
- **扫描提速**：六大 O(N²) 根因根治（逐条查询 / 循环内编译正则 / 全量重扫），大目录扫描时间与峰值内存大幅下降
