#!/usr/bin/env python3
"""UL10nForge 0.51.1 GitHub Release 发布。

0.51.0 是当天发布的缺陷版本（UI 消失 / rawstr 写回越界 / 审核假阳性），
本脚本用 0.51.1 替换它：删除 v0.51.0 release + tag，创建 v0.51.1
（文案 = 0.51.0 叙事 + 五项修复）并上传全部分卷。Models 分卷与
0.51.0 内容一致（模型未变），文件名重打 0.51.1 版本号。

    python scripts/_publish_0511.py                    # 只创建/更新 0.51.1 文案
    python scripts/_publish_0511.py --upload           # 文案 + 上传全部分卷
    python scripts/_publish_0511.py --replace-old      # 先删除 v0.51.0 release+tag
"""
...
TAG = "v0.51.1"
OLD_TAG = "v0.51.0"
RELEASE_NAME = "UL10nForge 0.51.1 — 工具第二个公开版本：全面升级"

BODY = """# UL10nForge 0.51.1 — 第二个公开版本

一个**完全离线**的 Unity 游戏汉化工作台：识别 → 翻译 → 审校 → 写回全流程，每一步都有确定性检查与证据留档。

> 本项目由一位编程与游戏汉化的**新手**借助 **AI 辅助**独立开发——欢迎反馈问题（附复现步骤 + 日志）。

距离首个公开测试版 **0.30.0**（2026-08-15）一个多月，期间迭代 20+ 个内部版本、修复大量真实游戏实测问题、回归测试增长到 **3445 项**。这一版的主线可以概括成三句话：**写回更安全、识别更全、跑得更快更稳**——外加一个全新的在线 API 模式。
...
## 🔧 0.51.0 → 0.51.1 修复（真实游戏实测问题）

- **UI 消失但可点击**：TMP 字体图集替换后材质 `_TextureWidth`/`_TextureHeight`/`_GradientScale` 未同步 → SDF 采样错位文本渲染为空——现已以 bundle 材质浮点为权威同步三件套
- **写回「字符串长度越界」中断整个游戏**：rawstr 定位器漂移时盲读盲写——现写前内容自校验，坏定位器按条拒绝进审计闸门，不再中断其余条目
- **Unity.Localization 条目 save_typetree 拒绝**：`type_descriptor` 空格三段形态（`TypeName Namespace Assembly`）未被识别——现已与逗号形态同查
- **审核「PARSE_ERROR → 不可发布」假阳性**：审核模型把 JSON 字符串定界符写成中文弯引号——现结构邻接定向修复（78+ 条实测样本全部恢复为正常裁决，内容零失真）
- **运行时字体兜底插件常驻部署**：不再因静态覆盖完整而跳过插件部署，缺字风险再降一级（模型包不变，仅代码更新）

## 📦 下载哪个包？

| 包 | 内容 | 适合谁 |
|---|---|---|
| `UL10nForge-0.51.1-Full.7z.001~004`（4 卷，共约 7.4 GB） | 全部四个模型 + 内置 Python + llama.cpp | **本地离线用户**（默认模式，数据不出本机，解压即用） |
| `UL10nForge-0.51.1-Lite.7z.001~002`（2 卷，共约 2.5 GB） | 不含大模型，只带重排模型（0.6B）+ 运行时 | **在线 API 用户**（翻译/审核/检索走云端接口） |
| `UL10nForge-0.51.1-Models.7z.001~002`（2 卷，共约 4.2 GB） | 三个大模型（翻译 1.8B / 审核 4B / 检索 0.6B） | Lite 用户转本地离线时补齐模型 |
...
def _delete_old_release() -> None:
    """删除 v0.51.0 release + tag（当天发布的缺陷版本，0.51.1 替换）。"""
    req = Request(
        f"https://api.github.com/repos/{REPO}/releases/tags/{OLD_TAG}",
        headers=_headers())
    try:
        ...
        if exc.code == 404:
            print(f"[skip] {OLD_TAG} release 不存在")
            return
        raise
    dele = Request(
        f"https://api.github.com/repos/{REPO}/releases/{release['id']}",
        headers=_headers(), method="DELETE")
    with urlopen(dele, timeout=60) as r:
        r.read()
    print(f"[ok] 已删除 release {OLD_TAG} (id={release['id']})")
    # release 删除后 tag 仍在——一并删除，避免残留指向旧提交
    dele = Request(
        f"https://api.github.com/repos/{REPO}/git/refs/tags/{OLD_TAG}",
        headers=_headers(), method="DELETE")
    try:
        with urlopen(dele, timeout=60) as r:
            r.read()
        print(f"[ok] 已删除 tag {OLD_TAG}")
    except HTTPError as exc:
        print(f"[warn] tag {OLD_TAG} 删除失败（{exc.code}），可手动清理")
...
def main() -> int:
    ...
    if "--upload" not in sys.argv:
        print("[done] release 已就绪；分卷请到网页手动上传"
              "（或加 --upload 自动上传）")
        return 0
    parts = sorted(DIST.glob("UL10nForge-0.51.1-*.7z.*"))
    if not parts:
        print("[FAIL] 分卷不存在：先运行 _package_0511.py")
        return 1
