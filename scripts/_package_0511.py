#!/usr/bin/env python3
"""UL10nForge 0.51.1 三通道发行打包（2026-09-09）。

复用 package.py 的白名单清单与隐私扫描，产出三个 7z 包（GitHub 单文件
< 2GB，超限自动分卷 .7z.001…）。Models 通道复用 0.51.0 已产出的分卷
（模型文件 0.51.0→0.51.1 无变化，代码更新不影响模型包）。

    python scripts/_package_0511.py              # 打包 + 验证 + SHA256
    python scripts/_package_0511.py --verify     # 只验证已有分卷完整性
    python scripts/_package_0511.py --reuse-models   # Models 复用 0.51.0 分卷（重命名）
"""
...
def _reuse_models() -> bool:
    """复用 0.51.0 Models 分卷（模型文件 0.51.0→0.51.1 无变化）：
    复制旧卷为 0.51.1 名并核对分卷数/校验。返回是否成功。
    """
    old = sorted(DIST.glob(f"UL10nForge-{OLD_VERSION}-Models.7z*"))
    if not old:
        print(f"[reuse] 没有 {OLD_VERSION} Models 分卷可复用，走重新打包")
        return False
    for p in old:
